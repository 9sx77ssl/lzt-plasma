...
Kirigami.FormLayout {
    ...
    property alias cfg_currencyRefreshInterval: currencyIntervalSpinBox.value
    ...
    QQC2.ComboBox {
        ...
        Component.onCompleted: {
            for (var i = 0; i < model.length; i++) {
                if (model[i].value === cfg_apiServer) {
                    currentIndex = i
                    return
                }
            }
    ...
    Kirigami.Separator {
        Kirigami.FormData.isSection: true
        Kirigami.FormData.label: i18n("Balance")
    }

    QQC2.SpinBox {
        id: intervalSpinBox
        Kirigami.FormData.label: i18n("Balance refresh (sec):")
        from: 5
        to: 3600
        value: 30
        stepSize: 5
    ...
    QQC2.ComboBox {
        ...
        Component.onCompleted: {
            for (var i = 0; i < model.length; i++) {
                if (model[i].value === cfg_displayCurrency) {
                    currentIndex = i
                    return
                }
            }
    ...
    QQC2.SpinBox {
        id: currencyIntervalSpinBox
        Kirigami.FormData.label: i18n("Currency rate refresh (sec):")
        from: 60
        to: 3600
        value: 300
        stepSize: 30
